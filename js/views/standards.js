/* =========================================================
   ATLAS · Standards view  (technology ↔ service mappings)
   ========================================================= */
(function () {
  ...
  async function render(host) {
    ...
    host.innerHTML = `
      <div class="view-header">
        <div class="toolbar">
          <input type="search" id="std-search" placeholder="Search services or technologies…" style="min-width:260px">
        </div>
      </div>

      <div id="std-stats" class="stats-bar"></div>

      <div class="taxonomy-board" id="std-board"></div>
    `;

    renderStats();
    renderBoard();

    host.querySelector('#std-search').oninput = e => {
  ...
  function renderBoard() {
    const board    = document.getElementById('std-board');
  ...
  function lcClass(s) {
    return ({ 'Core':'lc-core', 'Emerging':'lc-emerging', 'Declining':'lc-declining',
              'Special Use':'lc-special', 'Not Permitted':'lc-not', 'Pending':'lc-pending' })[s] || '';
  }
  ...
  window.atlasViews.standards = { render };
})();
